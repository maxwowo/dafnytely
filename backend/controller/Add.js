...
class Add {
  static async add_blood(body) {
    ...
    for (let i = 0; i < body.bloods.length; i++) {
      let unit = new Unit(Bank.get_next_id(), body.bloods[i].type, body.bloods[i].arrival_date, body.bloods[i].use_by_date, body.bloods[i].donor_id, body.bloods[i].lab_id);
      if (!unit.expired()) {
        new_units.push(unit);
      } else {
